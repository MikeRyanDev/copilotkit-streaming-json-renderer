...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    console.log("[copilotkit] langgraph /info status", infoResponse.status);
  } catch (error) {
  ...
  try {
    ...
    console.log(
      "[copilotkit] headers content-type",
    ...
    if (innerBody) {
      ...
      if (Array.isArray(innerBody?.context) && innerBody.context.length > 0) {
        console.log(
          "[copilotkit] first context",
          JSON.stringify(innerBody.context[0]),
        );
      }
      if (Array.isArray(innerBody?.messages) && innerBody.messages.length > 0) {
        console.log(
          "[copilotkit] first message",
          JSON.stringify(innerBody.messages[0]),
        );
      }
      ...
      console.log(
        "[copilotkit] inner agentSession",
        innerBody?.agentSession ?? null,
      );
      console.log(
        "[copilotkit] inner forwardedProps keys",
        innerBody?.forwardedProps
          ? Object.keys(innerBody.forwardedProps)
          : null,
      );
